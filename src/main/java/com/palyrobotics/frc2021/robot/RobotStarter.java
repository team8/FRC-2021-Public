...
public final class RobotStarter {
	...
	static {
		...
		try {
			...
			var offset = (Long) unsafeClass.getMethod("staticFieldOffset", Field.class).invoke(unsafe, loggerField);
			unsafeClass.getMethod("putObjectVolatile", Object.class, long.class, Object.class).invoke(unsafe, loggerClass, offset, null);
		} catch (Exception exception) {
			Log.warn(String.format("Failed to disable illegal access warning for Kryonet: %s", exception));
		}
	}
